...
| **Household — Paperless Hybrid (design)** | **IN PROGRESS** | T0–T7 all PASSED; T5/T6 PASSED; STOP gate cleared; T8/T9 next | 25 Aug 2026 |
...
**Immediate next task: stage T8 (dual-mailbox acquisition, Phase 4) + T9 (parallel observation, Phase 5) for Goose dispatch.** Write `GOOSE_TASK_HOUSEHOLD_T8_T9_DUAL_MAILBOX_PARALLEL_OBSERVATION.md` to `/app/agent-workdir/tasks/` per the design §14 + authoritative spec §7 (Gmail/attachment acquisition) + §19 Phase 4/5. T8 involves real Sarah OAuth + Michael's existing Apps Script — it carries its own approval gate and may require Michael-manual steps (Sarah OAuth authorisation, rclone remote setup). T9 runs old and new retrieval routes in parallel with practical acceptance tests (exact identifier, conceptual retrieval, current version, cross-subject exclusion, poor OCR review, not-found, source citation). T10 (cutover) remains gated on T7 = PASS (✅ delivered) + T9 clean. Conditions C1–C5 from T7 carry to T10.
