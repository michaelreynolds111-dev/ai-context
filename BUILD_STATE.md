...
**Last updated:** 27 August 2026 (Trello MCP wired into LibreChat — PASSED (GOOSE result exit-test-verified), task file retired to archive. Readonly Observer / File Investigator agent — BUILD COMPLETE and WORKING from phone; micha-ro OS-token boundary unachievable on Windows 11 Home (GOTCHAS §14), code-level guarantee operative. Household Paperless Hybrid design track: T0–T9 all passed; **T10 controlled retrieval cutover PASSED** (plan + verified rollback drill; live production remap separately gated on Michael sign-off); T11 (electricity/utility ledger) HELD. Claude usage analysis run (391 conv / 7,847 msgs) — gap report pending production. Production baselines drifted to pgvector 30,878 / Mongo 2,021 / 12 containers — ongoing activity, not task mutation.)
**Current phase:** Phase 9 — Cutover (§13)
Readonly Observer (File Investigator) agent — BUILD COMPLETE and WORKING from phone (operative code-level guarantee; micha-ro OS-token boundary documented as unachievable on Windows 11 Home). Household Administration Platform — Paperless Hybrid: T10 cutover PASSED (CUTOVER_AND_ROLLBACK_PLAN.md produced at agent-workdir/tasks/T10_ARTEFACTS/; rollback drill verified; C1–C5 carry to live remap); T8 hygiene/cleanup DONE (spec §7.2/§7.3 MailSarah promotion commit ab6f2bd; old namespace archived; Drive folder deleted; rclone wired); Michael's next action = LIVE Household Admin repoint (T10 production flip, separately gated); T11 (electricity/utility ledger) HELD for next session.
...
| **Household — Paperless Hybrid (design)** | **IN PROGRESS** | T0–T9 PASSED; **T10 PASSED** (plan+drill; live remap gated); T8 hygiene/cleanup DONE; T11 HELD | 27 Aug 2026 |
...
- **Docker stacks on host:** two independent Compose stacks — `librechat` (6 project containers; GitHub MCP server managed by Claude Desktop via `claude_desktop_config.json`, not a persistent container) and pre-existing `torbox-system` (7 containers, unrelated to AI build). VERIFIED: admin-panel shows as `clickhouse` (image hosted under ClickHouse GitHub org) — not an anomaly.
  - **SearXNG (Deep Research Agent, Task 1):** self-hosted web-search backend deployed at `~/searxng/` — own Compose project `searxng`, container `searxng` (restart unless-stopped), joined external `librechat_default` network so the LibreChat `api` container reaches it at `http://searxng:8080`. JSON output enabled (`format=json`), engines bing/duckduckgo/google-cse/wikipedia, zero API keys. Port `127.0.0.1:8080` loopback only (no LAN/WAN). Search + fetch scope (general/legal/workplace/technical only — no clinical/household-identity/family-law). PASSED 24 Aug 2026.
  - **DEFERRED — Docker credential-helper symlink (dangling):** `~/.docker/config.json` `credsStore: "desktop.exe"` points at dangling `/Docker/host/bin/docker-credential-desktop.exe`. Blocks all authenticated/private-registry pulls (public images still work via clean `DOCKER_CONFIG=/tmp/docker-nocreds`). Recommended fix: repoint symlink to `C:\Program Files\Docker\Docker\resources\bin\docker-credential-desktop.exe`. NOTE: master-plan §3.3 still lists "Tavily or Brave Search API key (Cluster 4)" — now SUPERSEEDED by the SearXNG decision (self-hosted, free, no API key).
- **Build Coordinator agent:** LibreChat agent configured with `deepseek-ai/DeepSeek-V4-Flash-0731`, 10-skill index in instructions, filesystem MCP tool. Agent created per Michael (manual UI step). Step 5 test PASSED (16 Aug 2026 — Build Coordinator read BUILD_STATE, followed agent-builder process, staged plan-executor v2 update and master plan §13b).
  - **Claude Desktop:** DELETED by Michael (18 Aug 2026). `claude_desktop_config.json` path pointer in ai-workspace is stale — consider removing on next sweep.
  - **GitHub MCP connector (`github-buildstate`):** registered in `librechat.yaml` + `mcp/mcp-servers.json` as read-only streamable-http to `https://api.githubcopilot.com/mcp/readonly`, `requiresOAuth:false`, static PAT auth via `GITHUB_MCP_PAT` pointer in `~/LibreChat/.env` (value never stored in git/repo/config). TEST-PASSED 25 Aug 2026 — reads `michaelreynolds111-dev/ai-context` (HTTP 200 + content; the prior token returned 403 "Resource not accessible by personal access token" because it lacked repo Contents-read). Local git through WSL2 remains the only approved write/push route. NOTE: the LibreChat handoff doc referenced the key as `GITHUB_PAT` but the live key is `GITHUB_MCP_PAT` — naming discrepancy in the doc, live key is correct.
- **Trello MCP (`trello`):** wired 27 Aug 2026 as stdio server via `@delorenj/mcp-server-trello` under `mcpServers:` in `librechat.yaml` (line 410), same pattern as Spotify/filesystem. `TRELLO_API_KEY` + `TRELLO_TOKEN` appended to `~/LibreChat/.env` (values never stored in git/repo/report). PASSED — GOOSE_RESULT_ADD_TRELLO_MCP.md: 2 env vars verified, container recreated, trello server initialised, 54 tools exposed. Task file retired to `tasks/archive/`. Michael recommended to rotate the Trello token (hygiene).
...
- 2026-08-27 [trello-mcp] [DONE] Wired delorenj/mcp-server-trello into LibreChat as stdio MCP server (`trello:` block at librechat.yaml line 410, TRELLO_API_KEY + TRELLO_TOKEN appended to ~/LibreChat/.env, api container recreated via force-recreate); verified 2 env vars present, server initialised, 54 tools exposed, no secret values in any output — evidence: GOOSE_RESULT_ADD_TRELLO_MCP.md (read and exit-test-verified this session, Status: PASSED)
- 2026-08-27 [trello-mcp] [DONE] Retired GOOSE_TASK_ADD_TRELLO_MCP.md to tasks/archive/ (removed the plaintext credential copy from the active workdir); GOOSE_RESULT_ADD_TRELLO_MCP.md retained in outputs/ — evidence: move_file_mcp_filesystem succeeded + list_directory_mcp_filesystem on /app/agent-workdir/tasks/archive showed the task file present after move
- 2026-08-27 [trello-mcp] [PLANNED] Recommended Michael rotates the Trello token (it previously sat in a scratch task file) — Trello → profile → Developer API Keys → token → regenerate; then update ~/LibreChat/.env — evidence: none yet (Michael action, recommended as hygiene)
- 2026-08-27 [claude-usage-analysis] [DONE] Claude usage analysis was fully run (outputs exist at ~/agent-workdir/claude-export/analysis/: 391 conversations, 7,847 messages, ~13MB, date range 2026-05-15 to 2026-08-14) although no GOOSE_RESULT file was written — evidence: read_text_file_mcp_filesystem on summary.json + usage_report.md this session
- 2026-08-27 [claude-usage-analysis] [PLANNED] Produce the LibreChat usage-gap report from the existing analysis outputs (routing/automation-opportunity insights) and write GOOSE_RESULT_CLAUDE_USAGE_ANALYSIS.md to close the cross-reference gap — evidence: none yet. Verify: gap report delivered + result file written to outputs/
- 2026-08-27 [readonly-observer-agent] [DONE] Confirmed GOTCHAS §14 already present and correct (Windows 11 Home cannot persist a scheduled-task credential for a non-interactive account — Register-ScheduledTask 5.1 returns Logon Mode Interactive/Background; LSP batch-logon right absent on Home SKU); adopt the verified code-level read-only guarantee as operative; optional docker-boot-orchestrator.ps1 if OS-token isolation is ever needed — evidence: read_text_file_mcp_filesystem on docs/GOTCHAS.md tail confirmed §14 section present (no new gotcha to append this session)

- 2026-08-27 [readonly-observer-agent] [DISCUSSED] micha-ro OS-token boundary final decision: NOT achievable on this Windows 11 Home build — v6 definitive Logon Mode: Interactive/Background despite -User/-Password User-set registration (Register-ScheduledTask 5.1 cannot store the credential; Windows Home lacks the LSP/batch-logon right). All v1–v6 diagnostic iterations recorded. Chose Option A: accept the verified CODE-level read-only guarantee as operative; drop the boot-task micha-ro hardening. — evidence: v6 console output (Logon Mode: Interactive/Background + FATAL guard) + GOOSE_RESULT hardening v6
- 2026-08-27 [readonly-observer-agent] [DONE] Readonly Observer agent CONFIRMED WORKING from Michael's phone — operative guarantee: endpoint loopback-only, deny-list EVERYTHING_EXCEPT_SENSITIVE, credential-pattern [REDACTED], zero write verbs, adversarial battery 22/22 (0 mutations/leaks), LibreChat bridge host.docker.internal:8941 (Desktop read works, .ssh refused). — evidence: user statement "agent works from my phone" + reconfigure result + bridge healthz 200
- 2026-08-27 [readonly-observer-agent] [PLANNED] Optional future hardening if ever needed: run the endpoint via the existing docker-boot-orchestrator.ps1 (boot-persistent, but runs as micha not micha-ro) if OS-token isolation is ever required — otherwise the code-level guarantee stands. — evidence: none yet (optional, deferred)
- 2026-08-26 [readonly-observer-reconfigure-scope] [PARTIAL] Flipped the readonly-observer endpoint from allow-list (Documents+Downloads) to the locked deny-list model (EVERYTHING_EXCEPT_SENSITIVE per §6): code in `C:\Users\micha\readonly-observer\src\server.js`, `selfcheck.mjs` updated, adversarial battery 22/22 PASS (Desktop/Documents/Downloads/D:\Data/C:\ readable; .ssh/.aws/.config/HouseholdDataRaw/archive/.env refused; credential content [REDACTED]; 0 mutations/leaks). LibreChat bridge verified: 5 verbs load, live Desktop read returns 36 entries, .ssh refused. ACL: added `micha-ro` ReadAndExecute-allow + Modify-deny on C:\Users\micha. SKILL.md updated (live + canonical `~/ai-context/skills/readonly-observer/`). TWO GAPS: (1) endpoint runs under `micha` NOT `micha-ro` (no ReadonlyObserverMCP scheduled task exists) — OS read-only guarantee relies on endpoint CODE until run under micha-ro; (2) C:\ and D:\ drive-root ACL grants need ELEVATED PowerShell (not applied). Evidence: GOOSE_RESULT_READONLY_OBSERVER_RECONFIGURE_SCOPE.md in outputs/ (verified via bridge, 26 Aug 2026).
- 2026-08-24 [deep-research-agent] [DONE] Deployed SearXNG as its own Compose project (`~/searxng/`), joined `librechat_default`, JSON search live, `api` container reaches `searxng:8080` (STATUS:200), loopback-only port, gitleaks clean — evidence: GOOSE_RESULT_SEARXNG_DEPLOY.md in agent-workdir/outputs/ (PASSED, 24 Aug 2026).
- 2026-08-24 [deep-research-agent] [DISCUSSED] Locked Deep Research agent scope (general/legal/workplace/technical, no sensitivity tag), SearXNG backend, and search+fetch tool shape with Michael — evidence: user statement (all three decisions confirmed in chat). Verify: GOOSE_TASK_SEARXNG_DEPLOY.md and GOOSE_TASK_DEEP_RESEARCH_AGENT.md in agent-workdir/tasks/ encode the locked decisions.
- 2026-08-24 [deep-research-agent] [PLANNED] Task 2 — build the SearXNG MCP adapter (`search_web` + `fetch_page`, SSRF-bounded), register `searxng-search` in librechat.yaml, stage the deep-research skill, and produce the manual agent-creation blueprint — evidence: GOOSE_TASK_DEEP_RESEARCH_AGENT.md staged in agent-workdir/tasks/, not yet executed.
- 2026-08-24 [deep-research-agent] [PLANNED] Defer the dangling Docker credential-helper symlink fix (`docker-credential-desktop.exe`) as a backlog item + add a §18 change-trigger / Cluster-4-SearXNG note to the master plan — evidence: staging note BACKUP_AI_MASTER_BUILD_PLAN.md.DEFERRED_DOCKER_CRED_HELPER.md in agent-workdir/staging-ai-context/docs/.
...
- 2026-08-24 [household-paperless-hybrid-design] [DONE] Located the authoritative spec at projects/household/HOUSEHOLD_ADMIN_PLATFORM_AUTHORITATIVE_BUILD_SPECIFICATION.md and wrote the 14-section design-only implementation design to /app/agent-workdir/HOUSEHOLD_ADMIN_PLATFORM_PAPERLESS_HYBRID_PRODUCTION_IMPLEMENTATION_DESIGN.md (authority chain §0.1, controlled vocabulary §0.2, conflict register C1–C5 + DEV1 §0.3) — evidence: design file present in agent-workdir (write_file_mcp_filesystem this session).
- 2026-08-24 [household-paperless-hybrid-design] [DONE] Ceased on the ADOPT_PAPERLESS_HYBRID architecture direction and recorded conflicts (C1–C5 + DEV1) rather than silently rewriting prior Household 02 docs; HYBRID_SUPERSET vocabulary reconciliation — evidence: design file §0.2/§0.3 this session.
- 2026-08-24 [household-paperless-hybrid-design] [DONE] Dispatched T0/T1/T2 Goose task files and verified all three result files against their exit tests (T0 PASSED verify-only with 3 OPEN items signed off; T1 PASSED ADR draft 10-item does-not-authorise list; T2 PASSED 15/15 acceptance checks) — evidence: GOOSE_RESULT_HOUSEHOLD_T0_PHASE0_CLOSURE.md, GOOSE_RESULT_HOUSEHOLD_T1_ARCHITECTURE_DECISION_RECORD.md, GOOSE_RESULT_HOUSEHOLD_T2_ADAPTER_PROTOTYPE.md in agent-workdir/outputs/.
- 2026-08-24 [household-paperless-hybrid-design] [DONE] Dispatched and verified T0-fix micro-task closing T0 OPEN 1 (inline provisional markers) — 2 sibling PROVISIONAL.md markers added, 3 metric JSONs byte-identical (sha256 unchanged) — evidence: GOOSE_RESULT_HOUSEHOLD_T0_FIX_INLINE_PROVISIONAL.md in agent-workdir/outputs/.
- 2026-08-24 [household-paperless-hybrid-design] [PLANNED] T3 canonical-identity/metadata-schema realisation (§1 canonical identity spine store, §2 Paperless metadata schema, §3 source/mailbox provenance) producing CANONICAL_IDENTITY_SCHEMA.md, PAPERLESS_METADATA_SCHEMA.md, SOURCE_AND_MAILBOX_PROVENANCE_MODEL.md — schema/design-only — evidence: none yet. Verify: GOOSE_RESULT_HOUSEHOLD_T3_*.md passes schema-only + no-mutation exit gates.
- 2026-08-24 [household-paperless-hybrid-design] [PLANNED] T4 governed-ingestion bridge pilot (§4) — first MUTATION task (ingestion of copies), gated on T3, carries its own approval gate — evidence: none yet. Verify: GOOSE_RESULT_HOUSEHOLD_T4_*.md passes source-preservation + idempotency + no-production-regression gates.
- 2026-08-24 [household-paperless-hybrid-design] [PLANNED] Later tasks T5 (indexing bridge, swap in production all-MiniLM-L6-v2 embedding), T7 (pilot acceptance + STOP gate), T8 (dual-mailbox acquisition), T9 (parallel observation), T10 (cutover, gated on T7+T9, retain fallback), T11 (electricity ledger), T12 (Actual Budget), T13 (legacy retirement, archive-first) — evidence: design file §14 task sequence. Verify: each task's own Goose exit test.
- 2026-08-25 [household-paperless-hybrid-design] [DONE] T7 pilot acceptance STOP gate PASSED_WITH_CONDITIONS — 12/12 acceptance criteria scored against named evidence sources (§20.1 ingestion 6/6 PASS from T4 spine+run JSON; §20.2 retrieval safety 6/6 PASS from T2 adapter evidence + T4 field-level corroboration); spine crosswalk reconciled directly from sqlite; conditions C1–C5 carried to T10 cutover — evidence: GOOSE_RESULT_HOUSEHOLD_T7_PILOT_ACCEPTANCE.md in agent-workdir/outputs/.
- 2026-08-25 [household-paperless-hybrid-design] [DONE] T5+T6 indexing bridge + security/recovery PASSED — T5: production all-MiniLM-L6-v2 model swapped in (T2 limitation closed), 22-column chunk schema, only {351,352} indexed, rebuildability proven; T6: security policy applied (localhost-only, read-only token, least-privilege indexer), recovery drills exercised (export/restore/crosswalk/rebuild/filter/citations); adapter hardened (stale field map fixed, doc 353 poor-OCR no longer mis-flagged as answer-ready); production baseline unchanged (pgvector 29,177 / Mongo 2,009 / 12 containers / MCP route) — evidence: GOOSE_RESULT_HOUSEHOLD_T5_T6_INDEXING_SECURITY_RECOVERY.md in agent-workdir/outputs/.
- 2026-08-25 [household-paperless-hybrid-design] [DONE] Build Coordinator verified both T7 and T5/T6 result files fresh via filesystem MCP against their exit tests — T7: 12/12 criteria table + PASS_WITH_CONDITIONS + spine reconciliation + conditions C1–C5; T5/T6: indexer selects exactly {351,352} + full chunk schema + rebuildability + recovery drills + production baseline unchanged — evidence: this session's read_text_file_mcp_filesystem calls on both result files.
- 2026-08-25 [household-paperless-hybrid-design] [PLANNED] Stage T8 (dual-mailbox acquisition, Phase 4) + T9 (parallel observation, Phase 5) for Goose dispatch — batched, gated on T7=PASS (delivered). T8 involves real Sarah OAuth + Michael's Apps Script, carries own approval gate. T10 cutover additionally gated on T9 clean — evidence: design §14 + spec §7/§19.
- 2026-08-26 [household-t8-t9] [DONE] T8 acquisition setup PASSED (Parts A/B + Part C Michael-manual: Sarah Apps Script deployed with account-context guard, trigger installed; new read-only `sarah_gmail_ro` remote created `drive.readonly`; register confirmed Sarah holds bills) — evidence: GOOSE_RESULT_HOUSEHOLD_T8_ACQUISITION_SETUP.md in agent-workdir/outputs/.
- 2026-08-26 [household-t8-t9] [DONE] T8D post-deployment verification PARTIAL — 5/6 §20.4 criteria PASS; criterion-2 (attachments linked to messages) NOT yet evidenceable because no attachment-bearing message had been acquired (10 messages all `multipart/alternative`, 0 attachments); messages byte-exact 10/10; no defect — evidence: GOOSE_RESULT_HOUSEHOLD_T8_POST_DEPLOYMENT_VERIFY.md.
- 2026-08-26 [household-t8-t9] [DONE] T9 parallel observation PARTIAL — 6/7 practical acceptance checks PASS; check-6 (honest NOT_FOUND) FAILED because the legacy Paperless fulltext stage leaked weak matches; new-route semantic stage already correct (0.45 threshold); documented pre-T10 blocker; production baseline unchanged — evidence: GOOSE_RESULT_HOUSEHOLD_T9_PARALLEL_OBSERVATION.md.
- 2026-08-26 [household-t9r] [DONE] T9R not-found remediation PASSED — derived `t2_adapter_notfound.py` (original untouched, SHA 1af5f14e…) added a fulltext token-coverage gate `FULLTEXT_MIN_TOKEN_COVERAGE=0.50`; all 7 acceptance checks now PASS incl. check-6 (absent queries → `not_found: true`); semantic 0.45 unchanged; entity-absence edge case deferred to register at T10 — evidence: GOOSE_RESULT_HOUSEHOLD_T9R_NOTFOUND_REMEDIATION.md.
- 2026-08-26 [household-t8-namespace] [DONE] Root cause of full-width-slash `Mail/sarah` namespace defect confirmed as design flaw in `getOrCreateNestedFolder_` (getFoldersByName treats `Mail/sarah` as one literal name, never splits on slash); both script constants confirmed clean ASCII; Sarah's data stranded in a flat `Mail／sarah` (U+FF0F) folder — evidence: GOOSE_RESULT_HOUSEHOLD_T8_NAMESPACE_DEFECT_REPAIR.md.
- 2026-08-26 [household-t8-namespace] [DONE] Michael reverted his script constant from temporary `MailMichael` back to `"Mail"` (zero migration, 24,356 emails untouched); Sarah set to `"MailSarah"` (flat, slash-free) — evidence: user confirmed both deployed scripts updated/saved on script.google.com (Michael reverted to "Mail", Sarah on "MailSarah").
- 2026-08-26 [household-t8-namespace] [DONE] T8 Sarah namespace migration PASSED — migrated Sarah's 25 messages + 5 attachments into canonical flat `MailSarah/` (Drive + `D:\Data\MailSarah\`); criterion-2 verified against the 5 real attachments (deterministic `msg<GmailId>_att<pos>` naming, `.eml` linkage to 2 Gmail IDs, Drive MD5 == local MD5); production baseline unchanged; Michael's `Mail/` + `Calendar/` untouched — evidence: GOOSE_RESULT_HOUSEHOLD_T8_SARAH_NAMESPACE_MIGRATE.md.
- 2026-08-26 [household-t8-t10] [DONE] T8 post-deployment confirmed FULL PASS after criterion-2 verified against real migrated attachments → T10 (cutover) now ELIGIBLE (T9 already clean 7/7). T10 requires its own explicit dispatch with CUTOVER_AND_ROLLBACK_PLAN.md; NOT auto-dispatched. — evidence: GOOSE_RESULT_HOUSEHOLD_T8_SARAH_NAMESPACE_MIGRATE.md (criterion-2 PASS) + T9R PASSED (T9 7/7).
- 2026-08-27 [household-t10] [DONE] T10 controlled retrieval cutover PASSED — CUTOVER_AND_ROLLBACK_PLAN.md produced (spec §21 item 13, at ~/agent-workdir/tasks/T10_ARTEFACTS/); rollback drill verified (old household-search MCP route intact as fallback, flip-back documented as config change not data change); derived adapter already proven 7/7 in T9R; baseline captured fresh (pgvector 30,878, Mongo 2,021, 12 containers); no production mutation; live production remap is a SEPARATE explicitly-approved step gated on Michael sign-off — evidence: GOOSE_RESULT_HOUSEHOLD_T10_CUTOVER.md in agent-workdir/outputs/.
- 2026-08-27 [household-t8-hygiene] [DONE] T8 hygiene reconcile PARTIAL→resolved — Part 1: spec §7.2/§7.3 updated Mail/sarah → MailSarah (committed ab6f2bd); Part 2: register seed all 7 rows re-routed MICHAEL → SARAH_AUTHORISED_HOUSEHOLD_SOURCE; Part 3-5: 37 old files archived, empty Drive Mail/sarah deleted, rclone_sync.ps1 wired (additive) — evidence: GOOSE_RESULT_HOUSEHOLD_T8_HYGIENE_RECONCILE.md + GOOSE_RESULT_HOUSEHOLD_T8_SIGNOFF_CLEANUP.md + commit ab6f2bd.
- 2026-08-27 [household-t8-signoff] [DONE] Michael sign-off decisions executed: (1) archive old D:\Data\Mail\sarah\ to dated backup, (2) ignore 4 unique vestige files (preserved in archive), (3) delete empty Drive Mail/sarah/, (4) wire daily rclone copy into rclone_sync.ps1, (5) LIVE Household Admin repoint = next action after cleanup (separately gated), (6) T11 held for next session — evidence: user statement (explicit decisions) + GOOSE_RESULT_HOUSEHOLD_T8_SIGNOFF_CLEANUP.md.
- 2026-08-27 [household-baseline-drift] [DONE] Recorded production baseline drift from prior recorded values — pgvector langchain_pg_embedding rows 29,177 → 30,878; Mongo files 2,009 → 2,021; containers 12 unchanged. Consistent with ongoing acquisition/activity, NOT task mutation (read-only verification in T10). — evidence: GOOSE_RESULT_HOUSEHOLD_T10_CUTOVER.md Part 1 baseline.
- 2026-08-25 [github-mcp-connector] [DONE] GitHub MCP 403 BLOCKED resolved — diagnostic confirmed the 403 was a PAT credential-scope problem, not config: MCP init/tools-list/header all PASS, but GitHub REST returned 403 "Resource not accessible by personal access token" (PAT lacked repo Contents-read on michaelreynolds111-dev/ai-context) — evidence: GOOSE_RESULT_GITHUB_MCP_BUILD.md in agent-workdir/outputs/ (read and verified by Build Coordinator this session).
- 2026-08-25 [github-mcp-connector] [DONE] Michael created a new fine-grained read-only PAT (Contents + metadata read on michaelreynolds111-dev/ai-context) and set it as the single `GITHUB_MCP_PAT=` value in `~/LibreChat/.env` — evidence: user confirmed "new PAT set" (value never pasted into chat, per credential rule; retest PASS confirms it works).
- 2026-08-25 [github-mcp-connector] [DONE] LibreChat `api` container recreated (up -d --force-recreate, not restart) and the GitHub MCP connector retested — the exact read that previously 403'd now returns HTTP 200 with file content; token value never printed/stored (len=93 recorded only); no regression (LibreChat /api/config 200, rag_api healthy, config unmodified) — evidence: GOOSE_RESULT_GITHUB_MCP_RESTART_AND_RETEST.md in agent-workdir/outputs/ (read and exit-test-verified by Build Coordinator this session).
- 2026-08-25 [github-mcp-connector] [DISCUSSED] No new environment gotcha recorded — the 403 was a GitHub credential scope, not an environment defect; no GOTCHAS_UPDATE.md written. The live key is `GITHUB_MCP_PAT` (handoff doc said `GITHUB_PAT`) — documented in the environment-facts section above — evidence: this session's reading of librechat.yaml + .env presence check + diagnostic result.



## NEXT STEP

**Immediate next task:** none blocking — the Readonly Observer (File Investigator) agent is BUILT and WORKING from Michael's phone. The read-only guarantee is enforced by the verified CODE layer (loopback-only, deny-list EVERYTHING_EXCEPT_SENSITIVE, credential-redaction, zero write verbs, adversarial battery 22/22, LibreChat bridge live). The micha-ro OS-token boundary was documented as unachievable on Windows 11 Home (scheduled-task credential storage fails across all Register-ScheduledTask parameter sets) — accepted as a known limitation per Option A. No further action required for this agent.

**Next available work (Household Paperless-Hybrid track, per latest event log):**
1. **LIVE Household Admin repoint (T10 production flip)** — Michael's chosen next action after cleanup completed. Deploy/connect the bounded (derived `t2_adapter_notfound.py`) adapter read-path and repoint Household Admin from the old `household-search` MCP to it, RETAIN the old route as fallback, observe errors. Gated on Michael sign-off + separate approved dispatch (use `~/agent-workdir/tasks/T10_ARTEFACTS/CUTOVER_AND_ROLLBACK_PLAN.md`).
2. **T11 — electricity/utility ledger (spec §13.4, Phase 7)** — first structured-fact vertical slice. HELD for the next session unless Michael reprioritises.
3. **T12 (Actual Budget proof of fit)** then **T13 (legacy retirement)** follow in document order. Conditions C1–C5 from T7 carry forward.

Other deferred items remain available per the REMAINING map should Michael wish to prioritise differently.
